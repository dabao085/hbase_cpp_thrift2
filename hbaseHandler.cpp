...
bool CHbaseHandler::connect()
{
	if (isConnected)
	{
		LOG(INFO) << "Already connected, don't need to connect it again" << endl;
		return true;
	}
	try 
	{
	...
	{
		LOG(ERROR) << "Connect Hbase error : " << tx.what() << endl;
		return false;
	}
...
bool CHbaseHandler::disconnect()
{
	if (!isConnected)
	{
		LOG(ERROR) << "Haven't connected to Hbase yet, can't disconnect from it" << endl;
		return false;
	}
	...
	if (NULL != transport)
	{
		...
		{
			LOG(ERROR) << "Disconnect Hbase error : " << tx.what() << endl;
			return false;
		}
	}
	else
						...
						const string &rowValue)
{
	if (!isConnected)
	{
		LOG(ERROR) << "Haven't connected to Hbase yet, can't put row" << endl;
		return false;
	}
...
bool CHbaseHandler::getRow(const std::string &tableName, const std::string &rowKey, std::string &resultStr)
{
	if (!isConnected)
	{
		LOG(ERROR) << "Haven't connected to Hbase yet, can't read data from it" << endl;
		return false;
	}
...
bool CHbaseHandler::getSeqValue(const std::string &tableName, const std::string &rowKey, std::string &columnValue)
{
	...
	try
	{
		...
		if(list.size() != 1)
		{
			LOG(ERROR) << "错误:存在多条相同的sequence序列" << endl;
			return false;
		}
//		std::vector<TColumnValue>::const_iterator iter = list.begin();
//		std::vector<TColumnValue>::const_iterator iterEnd = list.end();
